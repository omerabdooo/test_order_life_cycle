...
class BoundRemoteDataSourceImpl implements BoundRemoteDataSource{
  ...
  BoundRemoteDataSourceImpl({required this.apiService, required this.secureStorage});
 ...
 String? token = await getToken();
    var data = await apiService.get(
        endPoint: 'Bonds/Accountant/GetBonds?searchType=1&pageNumber=1&pageSize=10',
...
List<BoundEntitny> getBoundList(Map<String, dynamic> data) {
     return getListFromData(data, (item) => BoundModel.fromJson(item));
  }
  @override
  Future<UpdateBoundStateEntinty> updateBoundState(int boundId,String action)async {
    ...
    Map<String, dynamic> data = await apiService.patch(endPoint:'Bonds/Accountant/UpdateBondStatus?bondId=$boundId&action=$action' , data:
      formData
    , headers: {
      'Authorization': 'Bearer $token'
    });
    UpdateBoundStateEntinty addOrderEntity = UpdateBoundStateModel.fromJson(data);
